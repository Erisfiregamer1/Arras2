		...
		return function (ratio) {
...

			{
				...
				for (var _i7 = messages.length - 1; _i7 >= 0; _i7--) {
					...
					drawBar(_x26 - msg.len / 2, _x26 + msg.len / 2, _y + height / 2, height, color.black);
					// Draw the text
					ctx.globalAlpha = Math.min(1, msg.alpha);
					msg.textobj.draw(_text, _x26, _y + height / 2, height - 4, color.guiwhite, 'center', true);
					// Iterate and move
					_y += vspacing + height;
					if (msg.status > 1) {
...

			{
				...
				_gui.skills.forEach(function drawASkillBar(skill) {
					...
					if (cap) {
						...
						drawBar(_x27 + _height / 2, _x27 - _height / 2 + _len5 * ska(cap), _y2 + _height / 2, _height - 3 + config.graphical.barChunk, color.black);
						drawBar(_x27 + _height / 2, _x27 + _height / 2 + (_len5 - gap) * ska(cap), _y2 + _height / 2, _height - 3, color.grey);
						drawBar(_x27 + _height / 2, _x27 + _height / 2 + (_len5 - gap) * ska(level), _y2 + _height / 2, _height - 3.5, col);
						// Blocked-off area
						if (blocking) {
							ctx.lineWidth = 1;
							ctx.strokeStyle = color.grey;
							for (var j = cap + 1; j < _max; j++) {
								drawGuiLine(_x27 + (_len5 - gap) * ska(j), _y2 + 1.5, _x27 + (_len5 - gap) * ska(j), _y2 - 3 + _height);
							}
						}
						// Vertical dividers
						...
						for (var _j = 1; _j < level + 1; _j++) {
							drawGuiLine(_x27 + (_len5 - gap) * ska(_j), _y2 + 1.5, _x27 + (_len5 - gap) * ska(_j), _y2 - 3 + _height);
						}
						// Skill name
						_len5 = save * ska(_max);
						var textcolor = level == maxLevel ? col : !_gui.points || cap !== maxLevel && level == cap ? color.grey : color.guiwhite;
						text.skillNames[ticker - 1].draw(name, Math.round(_x27 + _len5 / 2) + 0.5, _y2 + _height / 2, _height - 5, textcolor, 'center', true);
						// Skill key
						text.skillKeys[ticker - 1].draw('[' + ticker % 10 + ']', Math.round(_x27 + _len5 - _height * 0.25) - 1.5, _y2 + _height / 2, _height - 5, textcolor, 'right', true);
						if (textcolor === color.guiwhite) {
							// If it's active
							global.clickables.stat.place(ticker - 1, _x27, _y2, _len5, _height);
						}
						// Skill value
						if (level) {
							text.skillValues[ticker - 1].draw(textcolor === col ? 'MAX' : '+' + level, Math.round(_x27 + _len5 + 4) + 0.5, _y2 + _height / 2, _height - 5, col, 'left', true);
						}
						// Move on 
				...
				if (_gui.points !== 0) {
					...
					text.skillPoints.draw('x' + _gui.points, Math.round(_x27 + _len5 - 2) + 0.5, Math.round(_y2 + _height - 4) + 0.5, 20, color.guiwhite, 'right');
				}
			}

			{
				...
				// Handle exp
				// Draw the exp bar
				drawBar(_x28, _x28 + _len6, _y3 + _height2 / 2, _height2 - 3 + config.graphical.barChunk, color.black);
				drawBar(_x28, _x28 + _len6, _y3 + _height2 / 2, _height2 - 3, color.grey);
				drawBar(_x28, _x28 + _len6 * _gui.__s.getProgress(), _y3 + _height2 / 2, _height2 - 3.5, color.gold);
				// Draw the class type
				text.class.draw('Level ' + _gui.__s.getLevel() + ' ' + mockups[_gui.type].name, _x28 + _len6 / 2, _y3 + _height2 / 2, _height2 - 4, color.guiwhite, 'center', true);
				_height2 = 14;
				_y3 -= _height2 + _vspacing2;
				// Draw the %-of-leader bar
				drawBar(_x28 + _len6 * 0.1, _x28 + _len6 * 0.9, _y3 + _height2 / 2, _height2 - 3 + config.graphical.barChunk, color.black);
				drawBar(_x28 + _len6 * 0.1, _x28 + _len6 * 0.9, _y3 + _height2 / 2, _height2 - 3, color.grey);
				drawBar(_x28 + _len6 * 0.1, _x28 + _len6 * (0.1 + 0.8 * (max ? Math.min(1, _gui.__s.getScore() / max) : 1)), _y3 + _height2 / 2, _height2 - 3.5, color.green);
				// Draw the score
				text.score.draw('Score: ' + util.handleLargeNumber(_gui.__s.getScore()), _x28 + _len6 / 2, _y3 + _height2 / 2, _height2 - 2, color.guiwhite, 'center', true);
				// Draw the name
				ctx.lineWidth = 4;
				text.name.draw(player.name, Math.round(_x28 + _len6 / 2) + 0.5, Math.round(_y3 - 10 - _vspacing2) + 0.5, 32, color.guiwhite, 'center');
			}

			{
				...
				roomSetup.forEach(function (row) {
					...
					row.forEach(function (cell) {
						...
						drawGuiRect(_x29 + j++ * _len7 / _W, _y4 + _i8 * _height3 / _H, _len7 / _W, _height3 / _H);
					});
					_i8++;
				});
				ctx.fillStyle = color.grey;
				drawGuiRect(_x29, _y4, _len7, _height3);
				minimap.forEach(function (o) {
					if (o[2] === 17) {
						ctx.fillStyle = mixColors(getColor(o[2]), color.black, 0.5);
						ctx.globalAlpha = 0.8;
						drawGuiRect(_x29 + o[0] / global.gameWidth * _len7, _y4 + o[1] / global.gameHeight * _height3, 1, 1);
					} else {
						ctx.strokeStyle = mixColors(getColor(o[2]), color.black, 0.5);
						ctx.lineWidth = 1;
						ctx.globalAlpha = 1;
						drawGuiRect(_x29 + o[0] / global.gameWidth * _len7 - 1, _y4 + o[1] / global.gameWidth * _height3 - 1, 3, 3, true);
						ctx.lineWidth = 3;
					}
				});
				ctx.globalAlpha = 1;
				...
				_x29 + player.x / global.gameWidth * _len7 - 1, _y4 + player.y / global.gameWidth * _height3 - 1, 3, 3, true);
				ctx.lineWidth = 3;
				ctx.fillStyle = color.black;
				drawGuiRect(_x29, _y4, _len7, _height3, true); // Border

				drawGuiRect(_x29, _y4 - 40, _len7, 30);
				lagGraph(lag.get(), _x29, _y4 - 40, _len7, 30, color.teal);
				gapGraph(metrics.rendergap, _x29, _y4 - 40, _len7, 30, color.pink);
				timingGraph(GRAPHDATA, _x29, _y4 - 40, _len7, 30, color.yellow);
				// Text
				text.debug[5].draw('Prediction: ' + Math.round(GRAPHDATA) + 'ms', _x29 + _len7, _y4 - 50 - 5 * 14, 10, color.guiwhite, 'right');
				text.debug[4].draw('Update Rate: ' + metrics.updatetime + 'Hz', _x29 + _len7, _y4 - 50 - 4 * 14, 10, color.guiwhite, 'right');
				text.debug[3].draw('Latency: ' + metrics.latency + 'ms', _x29 + _len7, _y4 - 50 - 3 * 14, 10, color.guiwhite, 'right');
				text.debug[2].draw('Client FPS: ' + metrics.rendertime, _x29 + _len7, _y4 - 50 - 2 * 14, 10, color.guiwhite, 'right');
				text.debug[1].draw('Server Speed: ' + (100 * _gui.fps).toFixed(2) + '%' + (_gui.fps === 1 ? '' : ' OVERLOADED!'), _x29 + _len7, _y4 - 50 - 1 * 14, 10, _gui.fps === 1 ? color.guiwhite : color.orange, 'right');
				text.debug[0].draw(serverName, _x29 + _len7, _y4 - 50, 10, color.guiwhite, 'right');
			}

			{
				...
				text.lbtitle.draw('Leaderboard:', Math.round(_x30 + _len8 / 2) + 0.5, Math.round(_y5 - 6) + 0.5, _height4 + 4, color.guiwhite, 'center');
				var _i9 = 0;
				lb.data.forEach(function (entry) {
					drawBar(_x30, _x30 + _len8, _y5 + _height4 / 2, _height4 - 3 + config.graphical.barChunk, color.black);
					drawBar(_x30, _x30 + _len8, _y5 + _height4 / 2, _height4 - 3, color.grey);
					var shift = Math.min(1, entry.score / max);
					drawBar(_x30, _x30 + _len8 * shift, _y5 + _height4 / 2, _height4 - 3.5, entry.barcolor);
					// Leadboard name + score 
					text.leaderboard[_i9++].draw(entry.label + ': ' + util.handleLargeNumber(Math.round(entry.score)), _x30 + _len8 / 2, _y5 + _height4 / 2, _height4 - 5, color.guiwhite, 'center', true);
					// Mini-image
...

			{
				...
				if (_gui.upgrades.length > 0) {
					...
					_gui.upgrades.forEach(function drawAnUpgrade(model) {
						if (_y6 > yo) yo = _y6;
						xxx = _x31;
						global.clickables.upgrade.place(_i10++, _x31, _y6, _len9, _height5);
						// Draw box
						ctx.globalAlpha = 0.5;
						ctx.fillStyle = getColor(colorIndex);
						drawGuiRect(_x31, _y6, _len9, _height5);
						ctx.globalAlpha = 0.1;
						ctx.fillStyle = getColor(-10 + colorIndex++);
						drawGuiRect(_x31, _y6, _len9, _height5 * 0.6);
						ctx.fillStyle = color.black;
						drawGuiRect(_x31, _y6 + _height5 * 0.6, _len9, _height5 * 0.4);
						ctx.globalAlpha = 1;
						...
						// Tank name
						text.upgradeNames[_i10 - 1].draw(picture.name, _x31 + 0.9 * _len9 / 2, _y6 + _height5 - 6, _height5 / 8 - 3, color.guiwhite, 'center');
						// Upgrade key
						text.upgradeKeys[_i10 - 1].draw('[' + getClassUpgradeKey(_ticker) + ']', _x31 + _len9 - 4, _y6 + _height5 - 6, _height5 / 8 - 3, color.guiwhite, 'right');
						ctx.strokeStyle = color.black;
						ctx.globalAlpha = 1;
						ctx.lineWidth = 3;
						drawGuiRect(_x31, _y6, _len9, _height5, true); // Border
						if (_ticker++ % 2) {
